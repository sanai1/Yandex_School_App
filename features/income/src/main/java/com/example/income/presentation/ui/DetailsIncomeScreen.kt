...
fun DetailsIncomeScreen(
    ...
    var date by remember {
        mutableStateOf(transactionDomain?.transactionDate?.toLocalDate() ?: LocalDate.now())
    }
    var time by remember {
        mutableStateOf(transactionDomain?.transactionDate?.toLocalTime() ?: LocalTime.now())
    }
    ...
    val categoryIncome = viewModel.categoryIncome.collectAsStateWithLifecycle()
    viewModel.updateCategoryIncome()
    val accountAll = viewModel.accountList.collectAsStateWithLifecycle()
    viewModel.updateAccounts()
    if (categoryIncome.value is VisibleData.Loading || accountAll.value is VisibleData.Loading) {
        LoadingVisible()
    } else if (categoryIncome.value is VisibleData.Success && accountAll.value is VisibleData.Success) {
        Column {
            AccountSelection(
                modifier = modifier.height(70.dp),
                selectedAccount = account
                    ?: (accountAll.value as VisibleData.Success<List<AccountDomain>>).data.first(),
                accountList = (accountAll.value as VisibleData.Success<List<AccountDomain>>).data
            ) { newAccount ->
                account = newAccount
            }
            CategorySelection(
                ...
                selectedCategory = category
                    ?: (categoryIncome.value as VisibleData.Success<List<CategoryDomain>>).data.first(),
                categoryList = (categoryIncome.value as VisibleData.Success<List<CategoryDomain>>).data
            ) { newCategory ->
        ...
        ErrorVisible(if (categoryIncome.value is VisibleData.Error) (categoryIncome.value as VisibleData.Error<List<CategoryDomain>>).type else (accountAll.value as VisibleData.Error).type)
    }
}